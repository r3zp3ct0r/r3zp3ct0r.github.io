...
export default function NotesPageClient() {
  ...
  useEffect(() => {
    async function fetchNotes() {
      try {
        setIsLoading(true)
        // Prefer public index shape; fallback to file under /public/notes/
        let resp = await fetch("/notes-index.json")
        if (!resp.ok) {
          resp = await fetch("/notes/index.json")
        }
        if (!resp.ok) {
        ...
        const notesData: Note[] = (data?.posts?.all || data?.posts || []) as Note[]
        ...
        const uniqueCategories = Array.from(
          new Set(
            sortedNotes
              .flatMap((note: Note) => Array.isArray(note.categories) ? note.categories : [])
              .filter(Boolean),
          ),
        ) as string[]
  ...
  useEffect(() => {
    ...
    if (selectedCategory) {
      filtered = filtered.filter((note) => Array.isArray(note.categories) && note.categories.includes(selectedCategory))
    }
